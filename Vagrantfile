...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
    ...
    config.vm.box = "vpuppet"

    # The url from where the 'config.vm.box' box will be fetched if it
    # doesn't already exist on the user's system.
    config.vm.box_url = "file:///Users/ssasan/dev/vbox/base.box"
    ...
    config.vm.synced_folder "../../bin", "/mnt/bin"
    ...
    config.vm.define "pm" do |master|
        master.vm.hostname = "pm"
        master.vm.network :private_network, ip: "192.168.33.10"
        master.vm.provision :puppet do |p|
            p.manifest_file = "master.pp"
            p.module_path = "modules"
            p.options = ["--templatedir","/vagrant/templates", "--verbose", "--debug"]
        end
    end

    config.vm.define "pa" do |agent|
        agent.vm.hostname = "pa"
